...
def test_default_execution_planes_and_invalid_override() -> None:
    ...
    invalid = endpoint.model_copy(update={"attributes": {"execution_plane": "elsewhere"}})
    with pytest.raises(EnforcementError, match="SITE or CONTROL_PLANE"):
        execution_plane(invalid)


@pytest.mark.asyncio
async def test_dispatcher_turns_invalid_plane_configuration_into_state_error() -> None:
    _, adapter, dispatcher, _ = prepare(execution_plane_value="elsewhere")

    with pytest.raises(ResponseStateError, match="SITE or CONTROL_PLANE"):
