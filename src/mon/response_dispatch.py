...
class ResponseDispatcher:
    """Route policy-approved responses to the correct execution plane."""
    ...
    @staticmethod
    def _plane(execution: ResponseExecution) -> EnforcementExecutionPlane:
        try:
            return execution_plane(execution.plan.enforcement_point)
        except EnforcementError as exc:
            raise ResponseStateError(str(exc)) from exc

    @staticmethod
    def _plan_plane(plan) -> EnforcementExecutionPlane:
        try:
            return execution_plane(plan.enforcement_point)
        except EnforcementError as exc:
            raise ResponseStateError(str(exc)) from exc
    ...
    async def _dispatch_locked(
        ...
        *,
        ...
        requested_at = existing.requested_at if existing is not None else utcnow()
